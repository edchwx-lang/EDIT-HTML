...
11. Import with `design final import`. Fact, coverage, or safety failures are diagnostics returned to Huashu; Edit HTML must not rewrite wording, DOM, CSS, charts, or interactions.
12. Run `render` and `validate`. The Instrumenter only inlines local resources, injects stable edit/source attributes, adds theme/editor compatibility metadata, and emits `artifact.html`.
