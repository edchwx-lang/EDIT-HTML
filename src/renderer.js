...
export function renderReport({ report, presentation, assets = new Map() }) {
  ...
  const title = report.nodes.find((node) => node.type === "section")?.title ?? "研究报告";
  const navigation = report.nodes.filter((node) => node.type === "section").map((node, index) =>
    '<a href="#' + escapeAttribute(node.nodeId) + '"><span>' + String(index + 1).padStart(2, "0") + '</span>' + escapeHtml(node.title) + '</a>'
  ).join("");
  const body = report.nodes.map((node, index) => renderNode(node, {
    report,
  ...
  return '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8">' +
    ...
    '<p class="eyebrow">EDIT HTML REPORT · V4</p><h1>' + escapeHtml(title) + '</h1>' +
...
function renderChart(dataset, sourceRefs) {
  ...
  const marks = rows.map((row, index) => {
    ...
    return '<button class="chart-row" type="button" data-chart-label="' + escapeAttribute(label) + '" data-chart-value="' + escapeAttribute(value) + '"><span>' + escapeHtml(label) + '</span><i class="chart-mark" data-chart-mark style="width:' + width.toFixed(2) + '%;background:var(--report-chart-' + ((index % 8) + 1) + ')"></i><b>' + escapeHtml(value) + '</b></button>';
  }).join('');
  return '<figure class="interactive-chart" data-chart-id="' + escapeAttribute(chartId) + '" data-source-ref="' + escapeAttribute(source) + '"><figcaption>数据对比</figcaption><div class="chart-stage"><div class="chart-selection-band"></div>' + marks + '</div>' +
