...
export async function instrumentV5Variant(projectDir, variantId) {
  ...
  const huashuReceipt = ["5.3.0", "5.3.1", "5.3.2", "5.4.0"].includes(siteManifest.packageVersion)
    ? await requireFrozenHuashuOutput(projectDir, variantId, "final")
    : null;
  if (["5.3.0", "5.3.1", "5.3.2", "5.4.0"].includes(siteManifest.packageVersion)) await requireV5FinalVerification(projectDir, variantId);
